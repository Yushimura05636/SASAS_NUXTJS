...
<Dialog as="div" class="relative z-50 lg:hidden" @close="sidebarOpen = false">
  <TransitionChild
    ...
    leave-to="opacity-0"
  >
    <div class="fixed inset-0 bg-teal-900/80" />
  </TransitionChild>
  ...
  <div class="fixed inset-0 flex">
    ...
    >
      <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1 bg-teal-800 shadow-lg rounded-r-lg h-screen">
        <div class="flex grow flex-col gap-y-3 overflow-y-auto text-white px-4 pb-4">
            ...
            <ul role="list" class="flex flex-1 flex-col gap-y-4">
                ...
                <div>
                  <a
                    ...
                    :class="{
                      ...
                      'text-white hover:bg-gray-700 hover:text-white': highlightedItem !== item.name
                    }"
                  ...
                  >
                    ...
                    <span class="ml-auto" v-if="item.subLinks?.length">
                      <template v-if="isChildVisible[item.name]">
                        <ChevronUpIcon class="h-4 w-4 text-white" aria-hidden="true" />
                      </template>
                      <template v-else>
                  ...
                  >
                    <ul v-if="isChildVisible[item.name]" class="pl-4">
                      <li v-for="subLink in item.subLinks" :key="subLink.name">
                        <a
                          :href="subLink.href"
                          class="flex items-center text-white hover:text-indigo-400 p-2 rounded-md transition duration-200"
                        >
              ...
              <li class="mt-auto">
                <a
                  ...
                  class="group flex items-center gap-x-2 rounded-lg p-2 text-sm font-semibold leading-5 text-white hover:bg-gray-700 hover:text-white transition duration-200"
                >
...
>
  <div class="flex grow flex-col gap-y-3 bg-teal-800 shadow-lg rounded-r-lg text-white px-2 pb-4 overflow-hidden">
    <div class="flex items-center justify-center h-16">
      <button @click="toggleSidebar" class="text-white hover:text-white focus:outline-none">
...
>
  <div
    ...
    class="fixed inset-0 lg:w-64 lg:bg-teal-800 lg:border lg:border-gray-700 z-50 shadow-lg rounded-lg overflow-x-auto overflow-y-auto"
  >
    <!-- Sidebar Header -->
    <div class="flex h-16 items-center justify-between bg-teal-850 shadow-md px-4 rounded-t-lg">
<img
  class="h-12 w-12 rounded-full object-cover"
  src="../img/LendCash_Logo-removebg-preview.png"
  :alt="name.company"
/>
<h1 class="text-lg font-bold text-white">{{ name.company }}</h1>
<button @click="toggleSidebar" class="text-white hover:text-white focus:outline-none">
      ...
      <ul role="list" class="flex flex-1 flex-col gap-y-2 px-4">
        <li v-for="item in menuItems" :key="item.name">
          ...
          >
            ...
            <span class="ml-auto" v-if="item.subLinks?.length">
              <template v-if="isChildVisible[item.name]">
                <ChevronUpIcon class="h-5 w-5 text-white" aria-hidden="true" />
              </template>
              <template v-else>
        ...
        <li class="mt-auto">
          <a
            href="/user_account/profile?settings=settings"
            class="group flex items-left justify-left rounded-lg p-2 text-md font-semibold leading-5 text-white hover:bg-gray-700 hover:text-white transition duration-200"
          >
            <Cog6ToothIcon class="h-6 w-6" aria-hidden="true" />
            <span>Settings</span>
          </a>
...
import { Bars3Icon, XMarkIcon, HomeIcon, InformationCircleIcon, Cog6ToothIcon, CreditCardIcon, UserCircleIcon, UserPlusIcon } from '@heroicons/vue/24/outline';
