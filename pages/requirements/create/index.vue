<template>
    <div class="container mx-auto py-10 px-4 max-w-lg">
      <h2 class="text-2xl font-semibold mb-6 text-gray-800">Create New Requirement</h2>
      <form @submit.prevent="createRequirement" class="bg-white p-6 rounded-lg shadow-lg">
...
import { apiService } from '~/routes/api/API';
  ...
  const createRequirement = () => {
    // Typically, this is where you'd send the form data to the backend.
    // After creation, redirect to the requirements view page.
  ...
  try {
    const response = await apiService.authRequirementsCreate({})
